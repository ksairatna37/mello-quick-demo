...
app.post("/api/livekit-token", async (req, res) => {
  ...
  if (!livekitUrl || !apiKey || !apiSecret) {
    return res.status(500).json({
      success: false,
      error: "Missing LiveKit config. Set LIVEKIT_URL, LIVEKIT_API_KEY, LIVEKIT_API_SECRET.",
    });
  }
  ...
  try {
    ...
    // Fixed room name for Hindi voice - agent and user join same room
    const roomName = "mello-hindi-voice";

    console.log(`[LiveKit] Creating room: ${roomName} for user: ${userId}`);
    ...
    console.log(`[LiveKit] Room created successfully: ${roomName}`);
